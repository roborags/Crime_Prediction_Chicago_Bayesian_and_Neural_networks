...
Crime_Mean = [[0 for y in range(26)] for x in range(13)]
Crime_SD = [[0 for y in range(26)] for x in range(13)]
Crime_Count = [[[0 for z in range(13)] for y in range(26)] for x in range(13)]
#Crime_Count = [[0 for x in range(26)] for y in range(13)]
Crime_Norm = [[[0 for z in range(13)] for y in range(26)] for x in range(13)]
Crime_Ind_Prob = [[[0 for z in range(13)] for y in range(26)] for x in range(13)]
Crime_Prob = [[0 for y in range(26)] for x in range(13)]
...
def Probabilty_Homicide_district(District):
		...
		#with open("Input_Test.csv") as Read_file:
			...
			for row in CSV_Read:
				if row[5] == "HOMICIDE":
					...
					Month_ID = Month(row)
					Crime_Count[Month_ID][District_ID][year] += 1
					#print("Crime_Count[",Month_ID,"][",District_ID,"][",year,"] = ",Crime_Count[Month_ID][District_ID][year])
					year_count += 1
			total_count += year_count		
			print("Input file Read : ",File_List_Input[year-1],"Crime Count =",year_count)
			year_count = 0
		Read_file.close()
	
	print("Input file reading finished , Total Count = ",total_count)
	...
	for month in range (1,13):		# for every month
		for district in range (1,26): 	# for every district
			...
			for year in range (1,13):
			#for year in range (1,2):	
				#if(Crime_Count[month][district][year] == 0):
					#div_count -= 1
				#else:
				Crime_Mean[month][district] += Crime_Count[month][district][year]
			#if(div_count != 0):
			Crime_Mean[month][district] /= 12
			#else:
				#Crime_Mean[month][district] = 0
			#print("Mean of row ",month," is ",Crime_Mean[month],"and Div is ",div_count)
			
			for year in range (1,13):
			#for year in range (1,2):
				if(Crime_Count[month][district][year] != 0):
					Crime_SD[month][district] += (Crime_Count[month][district][year]  - Crime_Mean[month][district])**2
			
			#if(div_count != 0):
			Crime_SD[month][district] /= 12
			#else:
				#Crime_SD[month][district] = 0
			
			#print("SD of row ",x," is ",Crime_SD[x],"and Div is ",div_count)
			
			for year in range (1,13):
			#for year in range (1,2):
				if Crime_Mean[month][district]!= 0  and Crime_SD[month][district]!=0:# and Crime_Count[month][district][year] !=0:
					Crime_Norm[month][district][year] = ((Crime_Count[month][district][year] - Crime_Mean[month][district]) / math.sqrt(Crime_SD[month][district]))
					#print("Crime_Norm",y,"-",x," ",Crime_Norm[y][x])
					#Crime_Norm[y][x] = MapNormalVal(Crime_Norm[y][x])			
				else:
					Crime_Norm[month][district][year] = 0
			
			for year in range (1,13):
			#for year in range (1,2):
				#Crime_Norm[month][district][year] = limitOutlier(Crime_Norm[month][district][year])
				if Crime_Mean[month][district]!= 0  and Crime_SD[month][district]!=0:#Crime_Norm[month][district][year]!=0:
					#Crime_Ind_Prob[month][district][year] = calculateProbability(Crime_Norm[month][district][year],Crime_Mean[month][district],Crime_SD[month][district])
					Crime_Ind_Prob[month][district][year] = MapNormalProb(Crime_Norm[month][district][year])
					Crime_Prob[month][district] += Crime_Ind_Prob[month][district][year]
					#Crime_Prob[month][district] *= calculateProbability(Crime_Norm[month][district][year],Crime_Mean[month][district],Crime_SD[month][district])
					#Crime_Norm[y][x] = MapNormalVal(Crime_Norm[y][x])			
				else:
					...
					#print("Normal_Outlier_Value = ",Crime_Norm[month][district][year])
	...
	for month in range (1,13):
		with open(File_List_Output_Stat[month-1],"w") as Write_file:
			...
			for district in range (1,26):
				...
				row += str(Crime_Mean[month][district]) + ","
				row += str(Crime_SD[month][district]) + ","
				row += '\n'
				Write_file.write(row)
				#print("3",row)
				row = ""
	...
	for month in range (1,13):
		with open(File_List_Output_Norm[month-1],"w") as Write_file:
			...
			for district in range (1,26):
				row+= str(district)+ ","
				for year in range (1,13):
					row += str(Crime_Norm[month][district][year]) + ","
				row += str(Crime_Mean[month][district]) + ","
				row += str(Crime_SD[month][district]) + ","
				row += '\n'
				Write_file.write(row)
	...
	for month in range (1,13):
		with open(File_List_Output_Prob[month-1],"w") as Write_file:
			...
			for district in range (1,26):
				...
				#print("3",row)
				row = ""
				#print("Norm Mean[",month,"][",district,"] = ",Crime_Mean[month][district],"SD[",month,"][",district,"] = ",Crime_SD[month][district])
		Write_file.close()
	
	print("Output file Indv Prob writing finished")
	
	with open("Ouput_Year_Prob.csv","w") as Write_file:
		row=""
		row += " "+","
		for month in range (1,13):
			row += str(month)+","
		...
		for district in range (1,26):
			...
			#for month in range (1,2):
				row += str(Crime_Prob[month][district]) + ","
			#row += str(Crime_Mean[month][district]) + ","
			#row += str(Crime_SD[month][district]) + ","
			row += '\n'
			Write_file.write(row)
			#print("3",row)
			row = ""
			#print("Prob Mean[",month,"][",district,"] = ",Crime_Mean[month][district],"SD[",month,"][",district,"] = ",Crime_SD[month][district])
	Write_file.close()
	
	print("Output file Prob writing finished")
	...
	for x in range (1,26):
		...
		for y in range (1,13):	
			if(Crime_Count[y][x] == 0):
				...
				Crime_Mean[x] += Crime_Count[y][x]
		if(div_count != 0):
			Crime_Mean[x] /= div_count
		else:
			Crime_Mean[x] = 0
		print("Mean of row ",x," is ",Crime_Mean[x],"and Div is ",div_count)
		for y in range (1,13):
			if(Crime_Count[y][x] != 0):
				Crime_SD[x] += (Crime_Count[y][x] - Crime_Mean[x])**2
		
		if(div_count != 0):
			Crime_SD[x] /= div_count
		else:
			Crime_SD[x] = 0
		...
		for y in range (1,13):
			if Crime_Mean[x]!= 0  and Crime_SD[x]!=0:
				Crime_Norm[y][x] = ((Crime_Count[y][x] - Crime_Mean[x]) / Crime_SD[x])
				#print("Crime_Norm",y,"-",x," ",Crime_Norm[y][x])
				#Crime_Norm[y][x] = MapNormalVal(Crime_Norm[y][x])			
			else:
				Crime_Norm[y][x] = 0
		...
		for y in range (1,13):
			Is_Outlier = False
			Is_Outlier = checkOutlier(Crime_Norm[y][x])
			if Is_Outlier == True and Crime_Norm[y][x]!=0:
				Crime_Prob[y][x] = calculateProbability(Crime_Norm[y][x],Crime_Mean[x],Crime_SD[x])
